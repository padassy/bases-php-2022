...
// si pas de résultats 
if (empty($nb)) { // $nb => 0 voir https://www.php.net/manual/fr/function.empty.php
...
<body>
    ...
    if (isset($erreur0)) :
    ?>
        <h2><?= $erreur0; // écriture raccourcie pour echo 
            ?></h2>
    ...
    ?>
        <h3><?= ($nb < 2) ? "$nb article" : "$nb articles"; // ternaire 
            ?></h3>
        <?php
        // tant qu'on a des articles, alias $item pour stocker la ligne en cours sous ce nom
        foreach ($datas as $item) :
        ?>
            <div class="article">
                <h4><?= $item['titrearticles'] ?></h4>
                <p><?= html_entity_decode($item['textearticles'], ENT_QUOTES); // pour réencoder les p et br
                    ?></p>
